 # -*- coding: utf-8 -*-
"""
Created on Fri Aug  8 15:01:24 2025
...
# emails = ["venu.tgos@gmail.com", "deepthi.vaka2306@gmail.com"]
emails = ["venu.tgos@gmail.com"]
...
def highlight_diff(row):
    # Assuming 'col1' and 'col2' are the columns to compare
    styles = [''] * len(row)
    if row['Rank_Curr_Buy_Price1'] != row['Rank_Prev_Buy_Price1']:
        # Apply a highlight color to both columns if they differ
        styles[row.index.get_loc('Rank_Curr_Buy_Price1')] = 'background-color: yellow'
        styles[row.index.get_loc('Rank_Prev_Buy_Price1')] = 'background-color: yellow'
    return styles
styled_df1  = final_df1.style.apply(highlight_diff, axis=1)
def highlight_diff(row):
    # Assuming 'col1' and 'col2' are the columns to compare
    styles = [''] * len(row)
    if row['Rank_Curr_SellPrice1'] != row['Rank_Prev_Sell_Price1']:
        # Apply a highlight color to both columns if they differ
        styles[row.index.get_loc('Rank_Curr_SellPrice1')] = 'background-color: yellow'
        styles[row.index.get_loc('Rank_Prev_Sell_Price1')] = 'background-color: yellow'
    return styles
styled_df2  = final_df2.style.apply(highlight_diff, axis=1)

with pd.ExcelWriter('FNO_Buy_Report '+datefor[0]+'.xlsx', engine="openpyxl") as writer:
    sheet_name = "Bool"
    # final_df.style.apply(lambda x: (x != final_df['Rank_Curr_Entry1']).map({True: 'background-color: red; color: white', False: ''}), subset=['Rank_Prev_Entry1'])
    # Export DataFrame content
    styled_df1.to_excel(writer, sheet_name=sheet_name)
    # Set backgrund colors depending on cell values
    sheet = writer.sheets[sheet_name]    
    cell_list = ['J','K','W','X','Y']
    cols_list = [ 'Curr_Day_Close%', 'Curr_Day_Volume%','5Day_Perf', '30Day_Perf', '90Day_Perf']
    for i in range(len(cell_list)):
        for cell, in sheet[f'{cell_list[i]}2:{cell_list[i]}{len(final_df) + 1}']: # Skip header row, process as many rows as there are DataFrames
            value = final_df1[cols_list[i]].iloc[cell.row - 2] # value is "True" or "False"
            # r,g,b = 92,184,0
            try:
                cell.fill = PatternFill("darkDown", start_color=('6ad200' if value > '0' else 'ff2800'))
            except:
                cell.fill = PatternFill("darkDown", start_color=('6ad200' if value > 0 else 'ff2800'))    
    
    # Apply wrap text to the header row (row 1)
    for cell in sheet[1]:  # Row 1 in Excel is index 1 in openpyxl
        cell.alignment = Alignment(wrap_text=True)
        cell.font = Font(bold=True)

with pd.ExcelWriter('FNO_Sell_Report '+datefor[0]+'.xlsx', engine="openpyxl") as writer:
    sheet_name = "Bool"
    # final_df.style.apply(lambda x: (x != final_df['Rank_Curr_Entry1']).map({True: 'background-color: red; color: white', False: ''}), subset=['Rank_Prev_Entry1'])
    # Export DataFrame content
    styled_df2.to_excel(writer, sheet_name=sheet_name)
    # Set backgrund colors depending on cell values
    sheet = writer.sheets[sheet_name]    
    cell_list = ['J','K','W','X','Y']
    cols_list = [ 'Curr_Day_Close%', 'Curr_Day_Volume%','5Day_Perf', '30Day_Perf', '90Day_Perf']
    for i in range(len(cell_list)):
        for cell, in sheet[f'{cell_list[i]}2:{cell_list[i]}{len(final_df) + 1}']: # Skip header row, process as many rows as there are DataFrames
            value = final_df2[cols_list[i]].iloc[cell.row - 2] # value is "True" or "False"
            # r,g,b = 92,184,0
            try:
                cell.fill = PatternFill("darkDown", start_color=('6ad200' if value > '0' else 'ff2800'))
            except:
...
def send_email(receiver_email, subject, body, attachment_path=None):
    sender_email = "venu.tgos@gmail.com"  # Replace with your email
    sender_password = "whlm udcx zuxd stpf"  # Use an App Password (not your regular password)
    try:
...
if __name__ == "__main__":
    receiver_email = emails  # Replace with recipient's email
    # receiver_email = ["venu.tgos@gmail.com"]
    subject = "FNO Levels Report "+datefor[0] 
    body = f"Hello Deepu, \n \n Please check the attachment to view your current Stock Entry status. \n \n This is just a test email coming from Python script for now. \U0001F604 \n \n Best Regards, \n Sudheer"
